...
refRadius=10;
refError=1.05;
maskScale=0.03%7/2048%.02%.03%.1%.03%.03%.1%0.03%.1;
sigmaMask=64;
bitsADC=15;
useHoloSupport=true;
...
filteredRef=maskfilter(refImage,mask,2.^nextpow2(size(refImage)*4));
...
%do deconvolution
deconv=wiener(gather(pad2size(cross,size(scatterImage))),pad2size(image,size(scatterImage)),10);
...
subplot(3,2,4);imagesc(real(start));axis square;colormap(flipud(gray));title('start');
 subplot(3,2,5);imagesc(abs(filteredDeconv));axis square;colormap(flipud(gray));title('filt dec');
