...
%         reconAbs=medfilt2(reconAbs);
    ...
    thresholdRelative = 0.008;%0.002%0.001;
...
%     reconBw = imopen((reconBw),strel('disk',5)); %5%25
    ...
    reconBw = imclose((reconBw),strel('disk',50));%50%60 %25
    ...
    %set the reference
    support(...
        1+  floor(end/2-refSize(1)/2)+  floor(diff(2)/2):...
        0+  ceil (end/2+refSize(1)/2)+  floor(diff(2)/2),...
        1+  floor(end/2-refSize(2)/2)+  floor(diff(1)/2):...
    ...
    start(...
        1+  floor(end/2-crossSize(1)/2)-  ceil(diff(2)/2):...
        0+  ceil (end/2+crossSize(1)/2)-  ceil(diff(2)/2),...
        1+  floor(end/2-crossSize(2)/2)-  ceil(diff(1)/2):...
