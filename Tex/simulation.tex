...
Als interessante Testszene wird eine wassergefüllte ikosaederförmige Lecithin-Membran mit dem Außenradius 375 \si{nm} und einer Dicke von 25 \si{nm} um deren Mittelpunkt vier kleinere Protein-Kugeln (Radius 90 \si{nm}) in den Eckpunkten eines Tetraeders angeordnet sind (\fref{fig:komplexmodel}) sowie ein disjunktes Xenon-Dodekaeder mit Außenradius 100 \si{nm} betrachtet. Die komplexen Brechzahlen sind in \fref{tab:brechzahl} aufgeführt.

\begin{table}
	\centering
	\begin{tabular}{lllll}
		\hline\hline
		Material						&Summenformel				&Dichte 			&$\beta$				&$\delta$ 	\\
		\hline
		Lecithin					 	&							& 					&						&			\\ 					
		Wasser						  	&							& 					&						&			\\
		Protein  						&							& 					&						&			\\ 
		...
		\hline\hline
	\end{tabular}
	\caption[Materialen für komplexe Austrittswelle]{Brechzahlen der Materialien, die für die Simulation der Austrittswelle eines komplexen Modellobjektes verwendet werden.}
	\label{tab:brechzahl}
\end{table}	
