...
Die Entfaltung wurde mit einer simulierten Austrittswelle der Referenz durchgeführt. Obwohl hierfür von einer vollständig bekannte Referenz ausgegangen wird, leidet die Rekonstruktion deutlich unter deren Irregularität und der Diskretisierung. Die IPR leidet hingegen sichtbar unter Zwillingsbildern. Dieses Problem kann durch die Kombination von IPR und Holographie umgangen werden. Das Ergebnis dieser Kombination unterscheidet sich optisch kaum von der Austrittswelle direkt hinter dem Objekt.

\begin{figure}
	\centering
	\quadimage{[width=.75\textwidth]{images/recon3d-v2.png}}
	\caption[Rekonstruktion einer Austrittswelle]{Die Rekonstruktion der Austrittswelle in \fref{fig:komplex}. Dargestellt ist jeweils das Ergebnis normiert auf das in a) dargestellte optimale Ergebnis mit der aus \fref{fig:komplex} bekannten Farbskala. In b) ist die Rekonstruktion mittels IPR mit Shrink-Wrap, in c) mittels Entfaltung und in d) das Ergebnis nach der Kombination aus IPR und Holographie dargestellt.}
	\label{fig:recon3d}
\end{figure}
