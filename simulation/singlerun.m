function run=singlerun(N,dx,dz,wavelength,objects,param)
    ...
    %% margin variation in only thibault
    if nargin>=6 && isfield(param,'margin')
        ...
        run.profiles_y.thibault=zeros(N/2,1);
        run.profiles_stdev.thibault=zeros(N/2,1);
        run.profiles_min.thibault=zeros(N/2,1);
        run.profiles_max.thibault=zeros(N/2,1);
        run.errors_rel.thibault=zeros(N/2,1);
        run.errors_abs.thibault=zeros(N/2,1);
        run.errors_rms.thibault=zeros(N/2,1);
