function varargout = beta_delta_rad(varargin)
    % BETA_DELTA_RAD MATLAB code for beta_delta_rad.fig
    % Explorer for beta, delta, radius multirun data
    
    % Last Modified by GUIDE v2.5 08-Jun-2016 18:01:44
    
    % Begin initialization code - DO NOT EDIT
    gui_Singleton = 1;
    gui_State = struct('gui_Name',       mfilename, ...
        'gui_Singleton',  gui_Singleton, ...
        'gui_OpeningFcn', @beta_delta_rad_OpeningFcn, ...
        'gui_OutputFcn',  @beta_delta_rad_OutputFcn, ...
        'gui_LayoutFcn',  [] , ...
        'gui_Callback',   []);
...
% --- Executes just before beta_delta_rad is made visible.
function beta_delta_rad_OpeningFcn(hObject, eventdata, handles, varargin)
...
function draw(handles)
    cla(handles.axes);
    beta=handles.ub(handles.list_beta.Value);
    delta=handles.ud(handles.list_delta.Value);
    ...
    legend(handles.axes,labels);
    drawnow;
end


% --- Outputs from this function are returned to the command line.
function varargout = beta_delta_rad_OutputFcn(hObject, eventdata, handles)
